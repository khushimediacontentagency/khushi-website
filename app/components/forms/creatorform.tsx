...
function SocialRow({ index, register, remove, showDelete, setVal, watch, errors, isSubmitting }: any) {
  ...
  return (
    <div className={`bg-white/5 p-4 rounded-xl border border-white/5 space-y-3 ${isSubmitting ? 'opacity-50 pointer-events-none' : ''}`}>
      <div className="flex flex-col md:flex-row gap-3 items-start">
        ...
        <div className="w-full md:w-1/4 min-w-[130px] flex gap-2">
            ...
            {showDelete && (
              <button disabled={isSubmitting} type="button" onClick={() => remove(index)} className="text-gray-500 hover:text-[#ff1267] p-2 transition-colors flex items-center justify-center bg-white/5 rounded-lg border border-white/10 h-[50px] w-[50px]">
                <FaTrash size={14}/>
              </button>
            )}
        </div>
      ...
      {platform === 'Other' && (
        <div className="animate-in fade-in slide-in-from-top-1 duration-200">
          <input disabled={isSubmitting} {...register(`socials.${index}.customPlatform`, { required: "Required" })} className={inputStyle(false)} placeholder="Platform Name (e.g. X, Snapchat)" />
        </div>
      )}
    </div>
  );
    ...
    <form onSubmit={handleSubmit((data) => { onSubmit(data, 'creator'); reset(); })} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div className="relative">
            <FaUser className="absolute left-3 top-1/2 -translate-y-1/2 text-[#ff1267] text-sm" />
            <input disabled={isSubmitting} {...register('lastName', { required: "Surname is required" })} className={inputStyle(!!errors.lastName, true)} placeholder="Surname" />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <div className="relative">
            ...
            <input disabled={isSubmitting} {...register('email', { required: "Email is required" })} className={inputStyle(!!errors.email, true)} placeholder="Email" />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         <div className={`w-full ${isSubmitting ? 'opacity-50 pointer-events-none' : ''}`}>
            <CustomSelect 
         ...
         {locationSelection === "Not Listed (Type Below)" && (
           <div className="w-full animate-in fade-in slide-in-from-left-2 duration-300">
              ...
              <div className="relative">
                ...
                <input disabled={isSubmitting} {...register('locationCustom', { required: "Please specify location" })} className={inputStyle(!!errors.locationCustom, true)} placeholder="e.g. Dubai" />
              </div>
      ...
      <div className={`bg-white/5 p-6 rounded-xl border border-white/5 ${isSubmitting ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          <div className="flex flex-col md:flex-row gap-6">
              <div className="flex gap-4 items-center min-w-[150px]">
                  <label className={`flex items-center gap-2 cursor-pointer p-3 rounded-lg border transition-all ${agencyStatus === 'Yes' ? 'border-[#ff1267] bg-[#ff1267]/10' : 'border-white/10 bg-white/5'}`}>
                      <input disabled={isSubmitting} type="radio" value="Yes" {...register('agencies', { required: "Please select" })} className="accent-[#ff1267]" />
                  ...
                  <label className={`flex items-center gap-2 cursor-pointer p-3 rounded-lg border transition-all ${agencyStatus === 'No' ? 'border-[#ff1267] bg-[#ff1267]/10' : 'border-white/10 bg-white/5'}`}>
                      <input disabled={isSubmitting} type="radio" value="No" {...register('agencies', { required: "Please select" })} className="accent-[#ff1267]" />
              ...
              {agencyStatus === 'Yes' && (
                  <div className="flex-1 w-full animate-in fade-in slide-in-from-left-2 duration-300">
                      <input disabled={isSubmitting} {...register('agencyList', { required: "Please list agencies" })} className={inputStyle(!!errors.agencyList)} placeholder="Please list them here..." />
      ...
      <div className="space-y-4">
        ...
        {fields.map((field, index) => (
          <SocialRow 
            key={field.id} index={index} register={register} remove={remove} control={control} showDelete={fields.length > 1} setVal={setValue} watch={watch} errors={errors} isSubmitting={isSubmitting}
          />
        ))}
        <button disabled={isSubmitting} type="button" onClick={() => append({ platform: '', link: '', followers: '' })} className={`text-[#ff1267] text-xs font-bold uppercase flex items-center gap-2 transition-colors ${isSubmitting ? 'opacity-50 cursor-not-allowed' : 'hover:text-white'}`}>
      ...
      <div>
        ...
        <textarea disabled={isSubmitting} {...register('details', { required: "Bio is required" })} className={`${inputStyle(!!errors.details)} min-h-36`} placeholder="Tell us about your work and creative journey..."></textarea>
      ...
      <button 
        disabled={isSubmitting} 
        type="submit" 
        className={`w-full bg-white text-black font-bold uppercase py-4 rounded-lg flex justify-center items-center gap-3 transition-all ${isSubmitting ? 'opacity-75 cursor-not-allowed' : 'active:scale-[0.98] hover:bg-gray-200'}`}
      >
        {isSubmitting ? (
          <>
